...
export function onSnapEndFinish(cg: Api, chess:any,delay: number,orig:any,dest:any,promote:any) {
  ...
  var undomove=false;
 if (move!==null && move.from.concat(move.to.substring(0,2))!== mv.substring(0,4) && !chess.in_checkmate()) { 	
    console.log(move.from.concat(move.to.substring(0,2)));
    console.log(mv);
    console.log(mv.substring(0,4))
    chess.undo();
    cg.set({
      fen:chess.fen(),
      turnColor: toColor(chess),
